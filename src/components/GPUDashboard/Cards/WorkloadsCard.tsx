...
const WorkloadsCard: React.FC = () => {
  ...
  const workloads = pods.reduce((acc, pod) => {
    const gpuMetrics = gpuMemoryMetrics?.[0]?.data?.result.find(
      (m) =>
    ...
    if (gpuUsage) {
      const podCPUMetric = getPodMetricValue(cpuMetrics?.[0], pod);
      const podMemoryMetric = getPodMetricValue(memoryMetrics?.[0], pod);
